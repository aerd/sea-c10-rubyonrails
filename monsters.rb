...
puts $monsters.map {|m| m[:dangers]}.flatten.inject(Hash.new(0)).{|h,d| h[d] += 1; h}.sort_by{|k,v| v}.last (2).map{|a| a [0]}
puts "What are the 2 most common vulnerabilities of our monsters?"
puts $monsters.map {|m| m[:vulnerabilities]}.flatten.inject(Hash.new(0)).{|h,d| h[d] += 1; h}.sort_by{|k,v| v}.last (2).map{|a| a [0]}
